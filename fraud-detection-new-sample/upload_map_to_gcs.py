...
def generate_map_and_upload_to_gcs(lat: float, lon: float, document_id):
    ...
    land_use_repo_mapbiomas = ee.Image('projects/mapbiomas-workspace/public/collection7_1/mapbiomas_collection71_deforestation_regeneration_v1')
    land_use_layers_2021_simplified = land_use_repo_mapbiomas.select('product_2021').divide(100).floor()
    full_brazil_geometry_with_10km_radius_hole = land_use_repo_mapbiomas.mask().geometry().difference(radius_10km_buffer)

    # display the full land use map
    vis_params = {'min': 1, 'max': 7, 'palette': [
      "#fffbc2", # 1, 'Anthropic'
    ...
    Map.to_html(filename=html_file, title='Land Use details', width='100%', height='300px')
